...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (start_orbit && IsMouseButtonDown(MOUSE_BUTTON_RIGHT)){
            ...
            camera.position.x = camera.target.x + cos(azimuth) * orbitRadius * cos(elevation);
            camera.position.z = camera.target.z + sin(azimuth) * orbitRadius * cos(elevation);

            // Vertical orbit (elevation)
            camera.position.y = camera.target.y + sin(elevation) * orbitRadius;
        }
